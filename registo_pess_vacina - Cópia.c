...
void CAD_VACINA(DADOS pessoa[],int ver_resp,int qtds_d){
    ...
    for (int i = 1; i <= max; i++)
    {
           ...
           comorblidades(pessoa,ver_resp,qtds_d,i);
...
que posteriormente seram utilizados para preencher o registo individual, esta função ainda não esta funcionando 
corretamente. usando STRCPY. Essa função copia um valor para uma variável de cadeia de caracteres.
*/
void preencher_RI(DADOS pessoa[], DADOS_IDT luc[]){
    for(int i=1;i<=max;i++){
...
void mostrar_Ri_por_parte(DADOS_IDT pessoa[], int opcao){
	system("cls");
...
void op_mostrar( DADOS_IDT ver[], int op_most);
...
void main(){
	...
	switch(opc){
		case 1:
            ...
            preencher_RI(Cidadao,aux);
            c=1;
        break;
        case 2:
        	if(c==1){
        		system("cls");
                op_mostrar(aux,op_most);
			}
			else{
				system("cls");
...
void op_mostrar( DADOS_IDT ver[], int op_most){
  for(;;){
    ...
    for (int i = 1; i <= max; i++)
    {
     ...
     switch(op_most){
         case 1:
             mostrar_Ri_por_parte(ver,op_most); 
         break;
         case 2:
             mostrar_Ri_por_parte(ver,op_most);
         break; 
